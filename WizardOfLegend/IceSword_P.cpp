...
CIceSword_P::CIceSword_P(const OBJ_TYPE& _eType)
	: CProjectile(_eType)
{
}


CIceSword_P::~CIceSword_P()
{
}

const bool CIceSword_P::Init()
{
	// Component Setting
	Create_Transform();
	Create_Collider();
...
void CIceSword_P::LateUpdate()
{
	if (!m_bActive) return;
	Component_LateUpdate();
}

void CIceSword_P::Render(HDC _hDC)
{
	if (!m_bActive) return;
	Component_Render(_hDC);
}

void CIceSword_P::Release()
{
}

void CIceSword_P::OnCollision_Enter(CCollider * _pOtherCol, const float & _fCX, const float & _fCY)
{
...
void CIceSword_P::OnCollision_Stay(CCollider * _pOtherCol, const float & _fCX, const float & _fCY)
{
}

void CIceSword_P::OnCollision_Exit(CCollider * _pOtherCol, const float & _fCX, const float & _fCY)
{
}

void CIceSword_P::Reset(CObj * _pOwner, const VEC2 & _vPos, const float & _fDamage, const float & _fTheta)
{
	m_pOwner = _pOwner;
...
void CIceSword_P::Return()
{
	CObjPool<CIceSword_P>::Return_Obj(this);
}

const bool CIceSword_P::Init_Animator()
{
	CTexture* pTex;
	...
	pTex = CResMgr::Get_Inst()->Load_Tex(L"ICE_SWORD", L"../Data/Sprite/04_Enemy/0_IceBoss/IceSword4.png");
...
void CIceSword_P::Explode()
{
	Return();
}

void CIceSword_P::Move()
{
