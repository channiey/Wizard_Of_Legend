...
#include "EventMgr.h"
CIceKatana_P::CIceKatana_P(const OBJ_TYPE& _eType)
	: CProjectile(_eType)
{
}


CIceKatana_P::~CIceKatana_P()
{
}

const bool CIceKatana_P::Init()
{
	// Component Setting
	Create_Transform();
	Create_Collider();
...
void CIceKatana_P::Render(HDC _hDC)
{
	if (!m_bActive) return;
...
void CIceKatana_P::Release()
{
}

void CIceKatana_P::Reset(CObj * _pOwner, const VEC2 & _vPos, const float & _fDamage, const float & _fTheta)
{
	m_pOwner = _pOwner;
...
void CIceKatana_P::Return()
{
	CObjPool<CIceKatana_P>::Return_Obj(this);

}

void CIceKatana_P::Explode()
{
	Return();
}

void CIceKatana_P::Move()
{
...
void CIceKatana_P::OnCollision_Enter(CCollider * _pOtherCol, const float & _fCX, const float & _fCY)
{
...
void CIceKatana_P::OnCollision_Stay(CCollider * _pOtherCol, const float & _fCX, const float & _fCY)
{
}

void CIceKatana_P::OnCollision_Exit(CCollider * _pOtherCol, const float & _fCX, const float & _fCY)
{
}

const bool CIceKatana_P::Init_Animator()
{
	CTexture* pTex;
	...
	pTex = CResMgr::Get_Inst()->Load_Tex(L"IceKatana", L"../Data/Sprite/04_Enemy/0_IceBoss/IceKatana0Large.png");
