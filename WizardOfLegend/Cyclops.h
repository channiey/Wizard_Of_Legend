...
class CCyclops : public CEnemy
{
public:
	CCyclops(const OBJ_TYPE& _eType);
	virtual ~CCyclops();
...
public:
	...
	void						Set_State(const CLOSE_ENEMY_STATE& _eState) { m_eState = _eState;  }
...
private:
	...
	float						m_fAttackTime; // 공격후 공격 투사체를 off할 시간
	float						m_fAccTime;
